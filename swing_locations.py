...
def player_miss_data(input_fields, player_io_df, player_ou_df):
    ...
    if ou_dict:
        ...
        test_ou['distance'] = test_ou['distance'].astype('float').fillna(np.clip(pd.Series(np.random.normal(1.25*net_over_val,2/3,size=test_ou.shape[0])),-2,2).astype('float'))
...
def miss_chart(combo_df,pos,b_hand,p_hand):
    ...
    for group in list(combo_df.loc[(combo_df['p_hand']==p_hand) & (combo_df['b_hand']==b_hand),group_base].value_counts().index):
        if pos=='p':
            bw_adjust = np.clip((swing_count_dict[(group,b_hand)])/30,1,2)
