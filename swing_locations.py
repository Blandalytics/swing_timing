...
def pull_day_misses(day):
    ...
    for pos in ["batter", "pitcher"]:
        ...
        date_dfs += [
            pd.read_csv(io.StringIO(res_miss_dist.decode("utf-8"))).assign(
                game_date=day, pos=pos
            )
        ]
...
def load_timing_data():
    ...
    with ThreadPoolExecutor(max_workers=16) as executor:
        futures = {
            executor.submit(pull_day_misses, day): day
            for day in pd.date_range(
                start=datetime(2026, 3, 25, tzinfo=ZoneInfo("America/New_York")).date(),
    ...
    miss_concise_df = (
        ...
        .rename(columns={"avg_x_flail": "avg_x_flailed"})
        .copy()
    )
    ...
    miss_concise_df["perfect"] = (
        miss_concise_df["perfect_percent"]
        .mul(miss_concise_df["n_swings"])
        .round(0)
        .astype("int")
    )
    for letter in ["x", "y", "z"]:
    ...
    for stat in timing_types:
        miss_concise_df[stat] = (
            miss_concise_df[stat + "_percent"]
            .mul(miss_concise_df["n_swings"])
            .round(0)
            .astype("int")
        )
...
hitter_list = [
    " ".join(x.split(", ")[::-1])
    for x in timing_data.loc[timing_data["pos"] == "batter", "name"]
    .sort_values(key=lambda x: x.str.lower())
    .unique()
]
pitcher_list = [
    " ".join(x.split(", ")[::-1])
    for x in timing_data.loc[timing_data["pos"] == "pitcher", "name"]
    .sort_values(key=lambda x: x.str.lower())
    .unique()
]
...
def player_bio_data(player_id: int) -> list[str]:
    ...
    return [
        json_data[0]["nameFirstLast"],
        json_data[0]["batSide"]["code"],
        json_data[0]["pitchHand"]["code"],
    ]
...
def combine_counts(x, y, pitchtype, p_hand, b_hand):
    ...
    return pd.DataFrame(
        itertools.product(
            x.loc[x.index.repeat(x["count"])]["distance"],
            y.loc[y.index.repeat(y["count"])]["distance"],
        ),
        columns=["in_out", "over_under"],
    ).assign(pitchtype=pitchtype, p_hand=p_hand, b_hand=b_hand, mul=group_mul)
...
def swing_sampling(test_inputs, player_io_df, player_ou_df):
    group_combos = {}
    max_len = 0
    for group, test_input in enumerate(test_inputs):
        date, pitch_type, zone_height, stand, throw = test_input
        test_io = player_io_df.loc[
            (player_io_df["game_date"] == date)
        ...
        if io_dict:
            if (io_dict["flailed"] + io_dict["tied_up"]) == 0:
                ...
                net_out_val = (io_dict["flailed"] - io_dict["tied_up"]) / (
                    io_dict["flailed"] + io_dict["tied_up"]
                )
            test_io["distance"] = (
                test_io["distance"]
                .astype("float")
                .fillna(
                    np.clip(
                        pd.Series(
                            np.random.normal(
                                4.0 * net_out_val, 4 / 2, size=test_io.shape[0]
                            )
                        ),
                        -4,
                        4,
        ...
        test_ou = player_ou_df.loc[
            (player_ou_df["game_date"] == date)
        ...
        if ou_dict:
            if (ou_dict["under"] + ou_dict["over"]) == 0:
                ...
                net_over_val = (ou_dict["under"] - ou_dict["over"]) / (
                    ou_dict["under"] + ou_dict["over"]
                )
            test_ou["distance"] = (
                test_ou["distance"]
                .astype("float")
                .fillna(
                    np.clip(
                        pd.Series(
                            np.random.normal(
                                1.25 * net_over_val, 1.25 / 2, size=test_ou.shape[0]
                            )
                        ),
                        -2,
                        2,
        ...
        first_list = np.concatenate(
            [a * [b] for a, b in zip(list(test_io["count"]), list(test_io["distance"]))]
        ).tolist()
        second_list = np.concatenate(
            [a * [b] for a, b in zip(list(test_ou["count"]), list(test_ou["distance"]))]
        ).tolist()
        combos = []
        ...
        if num_combos == 1:
            ...
            for draw in range(num_combos):
                combos += [
                    [
                        first_list.pop(np.random.randint(0, len(first_list))),
                        second_list.pop(np.random.randint(0, len(second_list))),
                    ]
                ]
...
def load_data(player_id, b_hand, p_hand, base_df=timing_data):
    ...
    in_out = ["tied_up", "centered", "flailed"]
    over_under = ["over", "lined_up", "under"]
    player_df = base_df.loc[
        (base_df["id"] == player_id)
        & (base_df["pos"] == ("pitcher" if pos == "p" else "batter"))
        & (base_df["pitch_hand"] == p_hand)
        & (base_df["bat_side"] == b_hand)
    ]
    if pos == "b":
        swing_count_dict = (
            player_df.groupby(["api_pitch_type", "pitch_hand"])["n_swings"]
            .sum()
            .to_dict()
        )
    else:
        swing_count_dict = (
            player_df.groupby(["api_pitch_type", "bat_side"])["n_swings"]
            .sum()
            .to_dict()
        )
    player_io_df = (
        pd.merge(
            player_df.melt(id_vars=group_cols, value_vars=in_out, value_name="count"),
            player_df.melt(
                id_vars=group_cols,
    ...
    player_ou_df = (
        pd.merge(
            player_df.melt(
                id_vars=group_cols, value_vars=over_under, value_name="count"
            ),
            player_df.melt(
                id_vars=group_cols,
                value_vars=["avg_z_" + x for x in over_under],
                value_name="distance",
    ...
    for permutation in range(max_permutations):
        permutation_combos.update(
            {permutation: swing_sampling(test_inputs, player_io_df, player_ou_df)}
        )
    test_combos = {}
    for y in range(len(test_inputs)):
        test_combos.update(
            {y: [[permutation_combos[x][y]][0] for x in permutation_combos]}
        )
    samples = {}
    ...
    sample_dfs = []
    for sample, sample_values in samples.items():
        date, pitch_type, zone_height, stand, throw = sample
        sample_dfs += [
            pd.DataFrame(sample_values, columns=["in_out", "over_under"]).assign(
                date=date,
    ...
    chart_df = pd.concat(sample_dfs, ignore_index=True).rename(
        columns={"stand": "b_hand", "throw": "p_hand", "pitch_type": "pitchtype"}
    )
...
def miss_chart(combo_df, pos, b_hand, p_hand):
    ...
    for group in combo_df.loc[
        (combo_df["p_hand"] == p_hand) & (combo_df["b_hand"] == b_hand), group_base
    ].unique():
        if pos == "p":
            ...
            bw_adjust = 1.5
        chart_df = combo_df.loc[
            (combo_df[group_base] == group)
            & (combo_df["p_hand"] == p_hand)
            & (combo_df["b_hand"] == b_hand)
        ].reset_index(drop=True)
        ...
        if pos == "p":
            fig.text(
                0.5,
                0.83,
                f"{b_hand}HH vs {player_name}'s {marker_names[group]}",
                ha="center",
                va="center",
            )
        else:
            ...
            fig.text(
                0.5,
                0.83,
                f"{player_name} vs {group.replace('_',' ')}{plural_text} ({p_hand}HP)",
                ha="center",
                va="center",
            )
        ...
        fig.add_artist(
            lines.Line2D(
                [0.125, 0.125], [0.11, 0.95], linewidth=3, color=chart_colors[group]
            )
        )
        fig.add_artist(
            lines.Line2D(
                [0.895, 0.895], [0.11, 0.95], linewidth=3, color=chart_colors[group]
            )
        )
        fig.add_artist(
            lines.Line2D(
                [0.125, 0.895], [0.11, 0.11], linewidth=3, color=chart_colors[group]
            )
        )
        fig.add_artist(
            lines.Line2D(
                [0.125, 0.895], [0.95, 0.95], linewidth=3, color=chart_colors[group]
            )
        )
        sns.kdeplot(
            chart_df,
        ...
        # Cap
        ax.plot([6, 6], [-0.49, 0.49], color="w", zorder=9, linewidth=2, alpha=0.5)

        ## Handle
        ax.plot(
            [-27, -27], [-2 / 2, 2 / 2], color="w", zorder=9, linewidth=2, alpha=0.5
        )
        ax.plot(
            [-28, -28], [-2 / 2, 2 / 2], color="w", zorder=9, linewidth=2, alpha=0.5
        )
        ax.plot(
            [-28, -27], [-2 / 2, -2 / 2], color="w", zorder=9, linewidth=2, alpha=0.5
        )
        ...
        ax.set(
            xlim=(-15, 15) if b_hand == "R" else (15, -15), ylim=(-7.5, 7.5), aspect=1
        )
        fig.text(0.87, 0.15, "Data: MLB", fontsize=9, ha="right")
        ...
        try:
            ...
            st.write(
                'Error generating chart. Please click "Generate Charts" button again'
            )
...
with col2:
    st.selectbox(f"Choose a {pos_text}:", player_list, index=player_idx, key="player")
    player_ids = list(
        timing_data.loc[
            (timing_data["pos"] == pos_text) & (timing_data["name"] == ss["player"]),
            "id",
        ]
        .value_counts()
        .index
    ...
    if len(player_ids) == 1:
        ...
        player_id = st.selectbox("Choose an MLBAMID:", player_ids)
...
with col3:
    if pos == "p":
        if p_hand == "S":
            p_hand = st.selectbox("Choose the player's pitching side:", ["R", "L"])
        hitter_handednesses = list(
            timing_data.loc[
                (timing_data["pos"] == pos_text)
                & (timing_data["id"] == player_id)
                & (timing_data["pitch_hand"] == p_hand),
                "bat_side",
            ]
            .value_counts()
            .index
        ...
        b_hand = st.selectbox(
            "Choose the opposing hitters' handedness:", hitter_handednesses
        )
    else:
        if b_hand == "S":
            b_hand = st.selectbox("Choose the player's hitting side:", ["R", "L"])
        pitcher_handednesses = list(
            timing_data.loc[
                (timing_data["pos"] == pos_text)
                & (timing_data["id"] == player_id)
                & (timing_data["bat_side"] == b_hand),
                "pitch_hand",
            ]
            .value_counts()
            .index
        ...
        p_hand = st.selectbox(
            "Choose the opposing pitchers' handedness:", pitcher_handednesses
        )
