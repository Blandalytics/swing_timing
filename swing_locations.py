...
def load_timing_data():
    ...
    with ThreadPoolExecutor(max_workers=16) as executor:
        futures = {executor.submit(pull_day_misses, day): day for day in pd.date_range(start=datetime.datetime(2026, 3, 25), end=datetime.datetime.now(ZoneInfo("US/Eastern")).date()-datetime.timedelta(days=1))}
...
def miss_chart(combo_df,pos,b_hand,p_hand):
    ...
    for group in combo_df.loc[(combo_df['p_hand']==p_hand) & (combo_df['b_hand']==b_hand),group_base].unique():
        ...
        if b_hand=='R':
            ...
            ax.plot([-12,-6.15], [0.9/2,2.58/2], color='w', zorder=9, linewidth=2, alpha=0.5)
