...
def miss_chart(combo_df,pos,b_hand,p_hand):
    ...
    for group in list(combo_df.loc[(combo_df['p_hand']==p_hand) & (combo_df['b_hand']==b_hand),group_base].value_counts().index):
        ...
        if b_hand=='R':
            ...
            ax.add_artist(mpatches.Arc((5.2,0.555),1.51,1.5,theta1=0, theta2=90,color='w',linewidth=2,alpha=0.5))
            ax.add_artist(mpatches.Arc((5.2,-0.58),1.51,1.5,theta1=270, theta2=360,color='w',linewidth=2,alpha=0.5))
